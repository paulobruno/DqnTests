...
from multiprocessing import Process, Manager, current_process, Lock
...
def get_best_action(model, state):
    ...
    a = tf.argmax(get_q_values(model, s)[0])
    print('.get_best_action')
    return a
...
def generate_memory(epoch, memory, q_network):
    ...
    while not episode_finished:
        ...
        print('action - %s.: %s' % (current_process().name, memory.get_size()))
        r = game.make_action(actions[a], frame_repeat)

        print('reward %f - %s.: %s' % (r, current_process().name, memory.get_size()))
    ...
    game.close()
    print('end %s' % (current_process().name))
...
if __name__ == '__main__':
    ...
    print('network')
    # initialize network
    q_network.predict(state.reshape([1, resolution[0], resolution[1], 1]))
    ...
    for epoch in range(num_epochs):    
        ...
        generate_memory(epoch, memory, q_network)
