...
from .relay_memory import ReplayMemory
import numpy as np
...
class DQN(keras.Model):
    ...
    def __init__(self, input_dim, output):
        ...
        self.model = self.create_network(input_dim, output)
    ...
    def create_network(self, inout_shape, output):
        dropout_prob = 0.3


        inputs = keras.Input(shape=inout_shape, name='frame')
        ...
        q = layers.Dense(output, kernel_initializer='glorot_normal', bias_initializer=tf.constant_initializer(0.1), activation=None)(x)
